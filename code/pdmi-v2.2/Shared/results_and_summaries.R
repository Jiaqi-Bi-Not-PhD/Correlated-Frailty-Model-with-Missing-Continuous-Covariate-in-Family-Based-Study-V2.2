...
v22_pack_result <- function(metadata, fit = NULL, pooled = NULL, penetrance = NULL,
                           diagnostics = list(), convergence = TRUE,
                           failure_reason = NA_character_) {
  omega <- if (!is.null(pooled)) pooled$omega else if (!is.null(fit)) fit$omega else setNames(rep(NA_real_, 5), v22_omega_names())
  V <- if (!is.null(pooled)) pooled$vcov_omega else if (!is.null(fit)) fit$vcov_omega else matrix(NA_real_, 5, 5)
  dimnames(V) <- list(v22_omega_names(), v22_omega_names())
  list(
    metadata = metadata,
...
v22_result_path <- function(metadata, config = v22_default_config()) {
  ...
  file.path(
    ...
    rate,
    v22_method_tag(metadata$method, metadata$prior_version),
    sprintf("replicate_%04d.rds", metadata$replicate_id)
  )
}

v22_is_once_only_benchmark <- function(metadata) {
  method <- tolower(metadata$method %||% "")
  method %in% c("full-data", "cca", "mi-smcfcs")
}

v22_existing_result_path <- function(metadata, config = v22_default_config()) {
  path <- v22_result_path(metadata, config)
...
v22_existing_benchmark_path <- function(metadata, config = v22_default_config()) {
  path <- v22_result_path(metadata, config)
  if (isTRUE(config$skip_existing_benchmarks) &&
      v22_is_once_only_benchmark(metadata) &&
      file.exists(path)) {
    return(path)
  }
  NA_character_
...
v22_write_result <- function(result, config = v22_default_config()) {
  path <- v22_result_path(result$metadata, config)
  dir.create(dirname(path), recursive = TRUE, showWarnings = FALSE)
  saveRDS(result, path)
  path
}

v22_summary_group_key <- function(...) {
...
v22_scenario_label <- function(missing_type, sigma_u2, target_missing_rate, prior_version = NA_character_) {
...
v22_rows_from_result <- function(result, true_omega = NULL) {
  md <- result$metadata
  true_omega <- true_omega %||% v22_actual_omega(md$sigma_u2)
  ...
  data.frame(
    ...
    M_imp = md$M_imp,
    parameter = v22_omega_names(),
    true = as.numeric(true_omega[v22_omega_names()]),
    estimate = as.numeric(result$omega[v22_omega_names()]),
    se = as.numeric(se[v22_omega_names()]),
...
v22_summarize_parameter_rows <- function(rows) {
  ok <- rows[is.finite(rows$estimate) & is.finite(rows$se) & rows$convergence, , drop = FALSE]
  if (!nrow(ok)) return(data.frame())
  split_key <- v22_summary_group_key(ok$missing_type, ok$sigma_u2, ok$target_missing_rate,
  ...
  do.call(rbind, lapply(pieces, function(d) {
    ...
    data.frame(
      scenario = v22_scenario_label(d$missing_type[1], d$sigma_u2[1],
...
v22_penetrance_rows_from_result <- function(result, config = v22_default_config()) {
  if (is.null(result$penetrance) || !nrow(result$penetrance)) return(data.frame())
  md <- result$metadata
  true_grid <- v22_penetrance_grid(v22_actual_omega(md$sigma_u2, config), config)
...
v22_summarize_penetrance_rows <- function(rows) {
  ok <- rows[is.finite(rows$estimate) & is.finite(rows$se) & rows$convergence, , drop = FALSE]
  if (!nrow(ok)) return(data.frame())
  split_key <- v22_summary_group_key(ok$missing_type, ok$sigma_u2, ok$target_missing_rate,
...
v22_manifest_parameter_table <- function(param_summary) {
...
v22_manifest_penetrance_table <- function(pen_summary) {
...
v22_flatten_diagnostics <- function(x, prefix = "diagnostic") {
  out <- list()
  walk <- function(y, nm) {
...
v22_diagnostic_rows_from_results <- function(results) {
  rows <- lapply(results, function(result) {
    ...
    as.data.frame(c(base, v22_flatten_diagnostics(result$diagnostics)), stringsAsFactors = FALSE)
  })
  Reduce(function(a, b) {
...
v22_summarize_diagnostics <- function(diagnostic_rows) {
  if (!nrow(diagnostic_rows)) return(data.frame())
  split_key <- v22_summary_group_key(diagnostic_rows$missing_type, diagnostic_rows$sigma_u2,
...
v22_penetrance_curve_means <- function(pen_rows) {
...
v22_penetrance_integrated_accuracy <- function(pen_rows) {
  ok <- pen_rows[is.finite(pen_rows$estimate) & pen_rows$convergence, , drop = FALSE]
  if (!nrow(ok)) return(data.frame())
  split_key <- v22_summary_group_key(ok$missing_type, ok$sigma_u2, ok$target_missing_rate,
...
v22_clean_filename <- function(x) {
  x <- gsub("[^A-Za-z0-9._-]+", "_", x)
  gsub("_+", "_", x)
}

v22_penetrance_plot_groups <- function(curve_means) {
  ...
  for (i in seq_len(nrow(scenarios))) {
    ...
    if (!nrow(d)) next
    priors <- unique(d$prior_version[
      d$method %in% c("C-O-PDMI", "C-R-PDMI") &
        !is.na(d$prior_version)
    ])
...
v22_write_penetrance_curve_figures <- function(curve_means, out_dir, config = v22_default_config()) {
  if (!nrow(curve_means)) return(character())
  dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
  groups <- v22_penetrance_plot_groups(curve_means)
  written <- character()
  colors <- c("full-data" = "#1F78B4", "Full-data" = "#1F78B4",
              "CCA" = "#D95F02", "MI-SMCFCS" = "#7570B3",
              "C-O-PDMI" = "#1B9E77", "C-R-PDMI" = "#66A61E")
  pchs <- c("full-data" = 16, "Full-data" = 16, "CCA" = 17,
            "MI-SMCFCS" = 15, "C-O-PDMI" = 18, "C-R-PDMI" = 8)
  for (grp in groups) {
    d <- grp$data
    if (!nrow(d)) next
    title <- v22_scenario_label(grp$missing_type, grp$sigma_u2,
                               grp$target_missing_rate, grp$prior_version)
    stem <- v22_clean_filename(paste("penetrance_curves", title, sep = "_"))
