...
##
## Prefer local V2 dependency-dump functions.  If an installed FamEvent
## namespace is needed for a helper missing from the local dump, import only
## that missing helper and never overwrite locally sourced functions.
## ============================================================
...
.famevent_import_if_missing <- function(symbols, target_env = .famevent_target_env) {
  ...
  suppressPackageStartupMessages(library(FamEvent))
