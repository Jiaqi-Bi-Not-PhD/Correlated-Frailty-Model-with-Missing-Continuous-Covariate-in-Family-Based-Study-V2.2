...
## Aggregate raw V2.2 replicate RDS files into parameter, penetrance,
...
v22_bind_rows <- function(x) {
  x <- x[vapply(x, nrow, integer(1)) > 0L]
  if (!length(x)) return(data.frame())
  do.call(rbind, x)
}

param_rows <- v22_bind_rows(lapply(res, v22_rows_from_result))
pen_rows <- v22_bind_rows(lapply(res, v22_penetrance_rows_from_result, config = config))
diagnostic_rows <- v22_diagnostic_rows_from_results(res)
param_summary <- v22_summarize_parameter_rows(param_rows)
pen_summary <- v22_summarize_penetrance_rows(pen_rows)
diagnostic_summary <- v22_summarize_diagnostics(diagnostic_rows)
curve_means <- v22_penetrance_curve_means(pen_rows)
integrated_accuracy <- v22_penetrance_integrated_accuracy(pen_rows)
...
utils::write.csv(v22_manifest_parameter_table(param_summary), file.path(out_dir, "parameter_manuscript_table.csv"), row.names = FALSE)
...
utils::write.csv(v22_manifest_penetrance_table(pen_summary), file.path(out_dir, "penetrance_manuscript_table.csv"), row.names = FALSE)
...
figures <- v22_write_penetrance_curve_figures(curve_means, fig_dir, config)
