...
## ============================================================
## Complete-data generator aligned to the V2 technical note.
## Complete families and covariates are generated first; missingness is
## applied only by Shared/missingness_and_cca.R.
## ============================================================

v2_make_kinship <- function(dat, id_col = "indID", father_col = "fatherID", mother_col = "motherID") {
...
v2_generate_mendelian_genotypes <- function(dat, pm = 0.02,
                                            id_col = "indID",
                                            father_col = "fatherID",
                                            mother_col = "motherID") {
  ids <- as.character(dat[[id_col]])
...
v2_generate_prs <- function(dat, K, sigma2_prs = 0.1) {
  out <- numeric(nrow(dat))
  blocks <- v2_family_blocks(dat)
  for (idx in blocks) {
    Ki <- as.matrix(K[idx, idx, drop = FALSE])
    out[idx] <- v2_rmvnorm_cov(rep(0, length(idx)), sigma2_prs * Ki)
  }
  out
}

v2_regenerate_event_times <- function(dat, K, omega, config = v2_default_config()) {
  th <- v2_theta_from_omega(omega)
  U <- numeric(nrow(dat))
  blocks <- v2_family_blocks(dat)
  for (idx in blocks) {
    Ki <- as.matrix(K[idx, idx, drop = FALSE])
    U[idx] <- v2_rmvnorm_cov(rep(0, length(idx)), th$sigma_u2 * Ki)
  }
  eta <- th$beta_c * as.numeric(dat$newx) + th$beta_b * as.numeric(dat$mgene) + U
  event_time <- th$lambda * (-log(stats::runif(nrow(dat))) / v2_safe_exp(eta))^(1 / th$rho)
...
v2_prepare_pedigree_copy <- function(full_dat, analysis_dat) {
  ped <- full_dat
...
v2_generate_candidate_family <- function(fam_id, cumind, sigma_u2, config = v2_default_config()) {
  ...
  full$geno_A_count <- v2_generate_mendelian_genotypes(full, pm = config$pm)
  ...
  K <- v2_make_kinship(analysis)
  analysis$newx <- v2_generate_prs(analysis, K, sigma2_prs = config$prs_sigma2)
  omega <- v2_actual_omega(sigma_u2, config)
  analysis <- v2_regenerate_event_times(analysis, K, omega, config)
  ped <- v2_prepare_pedigree_copy(full, analysis)
  list(dat = analysis, K = K, pedigree_dat = ped, full_dat = full)
}

v2_check_popplus_support <- function(dat) {
  blocks <- v2_family_blocks(dat)
...
v2_generate_complete_data <- function(replicate_id, sigma_u2, seed,
  ...
  cumind <- 0L
  attempts <- 0L
  max_attempts <- max(1000L, 400L * config$n_families)
  while (fam_id <= config$n_families) {
    attempts <- attempts + 1L
    if (attempts > max_attempts) {
      stop("Exceeded max_attempts while generating selected pop+ families.")
    }
    cand <- tryCatch(v2_generate_candidate_family(fam_id, cumind, sigma_u2, config),
  ...
  if (!v2_check_popplus_support(dat)) stop("Generated data violate pop+ support.")
  list(
