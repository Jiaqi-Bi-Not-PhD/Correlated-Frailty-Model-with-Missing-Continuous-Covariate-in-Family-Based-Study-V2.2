...
v2_safe_exp <- function(x, lim = 700) exp(pmax(pmin(x, lim), -lim))

v2_inv_logit <- function(x) 1 / (1 + exp(-pmax(pmin(x, 35), -35)))

v2_log_sum_exp <- function(x) {
  m <- max(x)
  if (!is.finite(m)) return(-Inf)
  m + log(sum(exp(x - m)))
}

v2_safe_chol <- function(A, jitter = 1e-8, max_tries = 8L) {
  A <- as.matrix(A)
  A <- 0.5 * (A + t(A))
...
v2_near_psd <- function(A, eps = 1e-8) {
  A <- 0.5 * (A + t(A))
  ee <- eigen(A, symmetric = TRUE)
...
v2_matrix_sqrt_sym <- function(A, inverse = FALSE, eps = 1e-10) {
  A <- 0.5 * (A + t(A))
  ee <- eigen(A, symmetric = TRUE)
  lam <- pmax(ee$values, eps)
...
v2_gh_rule <- function(order = 20L) {
  ...
  stop("v2_gh_rule() currently supports order 10 or 20.")
}

v2_H0 <- function(t, omega, agemin = 0) {
  th <- v2_theta_from_omega(omega)
  td <- pmax(as.numeric(t) - agemin, 0)
  (td / th$lambda)^th$rho
}

v2_H0_diff <- function(time, t0, omega, agemin = 0) {
  pmax(0, v2_H0(time, omega, agemin) - v2_H0(t0, omega, agemin))
}

v2_log_h0 <- function(time, omega, agemin = 0) {
  th <- v2_theta_from_omega(omega)
  td <- pmax(as.numeric(time) - agemin, 1e-12)
  log(th$rho) - th$log_lambda + (th$rho - 1) * (log(td) - th$log_lambda)
}

v2_alpha_popplus <- function(age, x_c, x_b, k_diag, omega, agemin = 0, gh_order = 20L,
                             eps = 1e-12) {
  th <- v2_theta_from_omega(omega)
  gh <- v2_gh_rule(gh_order)
  Hc <- v2_H0(age, omega, agemin)
  v <- pmax(th$sigma_u2 * k_diag, 1e-12)
  eta <- th$beta_c * x_c + th$beta_b * x_b
  d <- Hc * v2_safe_exp(eta + sqrt(2 * v) * gh$nodes)
  S <- sum((gh$weights / sqrt(pi)) * exp(-pmin(d, 745)))
  pmax(pmin(1 - S, 1 - eps), eps)
}

v2_penetrance <- function(age, x_c, x_b, omega, k0 = 1, agemin = 0, gh_order = 20L) {
  th <- v2_theta_from_omega(omega)
  gh <- v2_gh_rule(gh_order)
  Ht <- v2_H0(age, omega, agemin)
  v <- pmax(th$sigma_u2 * k0, 1e-12)
  eta <- th$beta_c * x_c + th$beta_b * x_b
  d <- Ht * v2_safe_exp(eta + sqrt(2 * v) * gh$nodes)
  1 - sum((gh$weights / sqrt(pi)) * exp(-pmin(d, 745)))
}

v2_penetrance_grid <- function(omega, config = v2_default_config()) {
  grid <- expand.grid(
...
v2_penetrance_gradient_omega <- function(age, x_c, x_b, omega, k0 = 1, agemin = 0,
                                         gh_order = 20L) {
  th <- v2_theta_from_omega(omega)
  gh <- v2_gh_rule(gh_order)
  td <- pmax(age - agemin, 0)
  if (td <= 0) return(setNames(rep(0, 5), v2_omega_names()))
  Ht <- v2_H0(age, omega, agemin)
  v <- pmax(th$sigma_u2 * k0, 1e-12)
  q <- sqrt(2 * v) * gh$nodes
  d <- Ht * v2_safe_exp(th$beta_c * x_c + th$beta_b * x_b + q)
  ...
  setNames(as.numeric(out), v2_omega_names())
}

v2_slice_univariate <- function(f, x0, w = 0.7, m = 80L, lower = -Inf, upper = Inf) {
  fx0 <- f(x0)
  if (!is.finite(fx0)) {
    x0 <- 0
    fx0 <- f(x0)
...
v2_family_blocks <- function(dat) split(seq_len(nrow(dat)), as.factor(dat$famID))

v2_align_K <- function(K, dat, id_col = "indID") {
...
v2_precompute_K_blocks <- function(K, dat, jitter = 1e-8) {
  blocks <- v2_family_blocks(dat)
  lapply(blocks, function(idx) {
    Ki <- 0.5 * (as.matrix(K[idx, idx, drop = FALSE]) + t(as.matrix(K[idx, idx, drop = FALSE])))
    R <- v2_safe_chol(Ki, jitter = jitter)
    list(idx = idx, K = Ki, K_inv = chol2inv(R),
         logdetK = 2 * sum(log(diag(R))), n = length(idx))
  })
}

v2_proband_indices <- function(dat) {
  blocks <- v2_family_blocks(dat)
